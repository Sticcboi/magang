...
$page_key = 'pkk';
$page_title = 'Kelola Kelembagaan PKK';
$current_page = 'admin_kelembagaan_pkk';
...
if (isset($_POST['update_content'])) {
    ...
    if ($exists && mysqli_num_rows($exists)) {
        mysqli_query($conn, "UPDATE kelembagaan_pages SET overview='$overview', visi='$visi', misi='$misi', legal_basis='$legal_basis', work_area='$work_area', notes='$notes' WHERE page = '$page_key'");
    } else {
        mysqli_query($conn, "INSERT INTO kelembagaan_pages (page, overview, visi, misi, legal_basis, work_area, notes) VALUES ('$page_key', '$overview', '$visi', '$misi', '$legal_basis', '$work_area', '$notes')");
    }
    $_SESSION['flash_msg'] = 'Konten PKK berhasil disimpan.';
    header("Location: admin_kelembagaan_pkk.php#konten?status=terhapus");
    exit;
}
...
if (isset($_POST['save_staff'])) {
    ...
    if ($staff_id) {
        ...
        $_SESSION['flash_msg'] = 'Anggota kepengurusan PKK berhasil ditambahkan.';
    }
    header("Location: admin_kelembagaan_pkk.php#staff?status=terhapus");
    exit;
}
...
if (isset($_POST['save_unit'])) {
    ...
    if ($unit_id) {
        ...
        $_SESSION['flash_msg'] = 'Unit pengelola PKK berhasil ditambahkan.';
    }
    header("Location: admin_kelembagaan_pkk.php#unit-program?status=terhapus");
    exit;
}
...
if (isset($_POST['save_program'])) {
    ...
    if ($program_id) {
        ...
        $_SESSION['flash_msg'] = 'Program kerja PKK berhasil ditambahkan.';
    }
    header("Location: admin_kelembagaan_pkk.php#unit-program?status=terhapus");
    exit;
}
...
<div class="container-fluid mt-4">
    <div class="hero-bkm-admin text-center mb-5">
        <div style="position: relative; z-index: 2;">
            ...
            <h1 class="display-5 fw-bold mb-3 text-white">Kelola PKK Kedungpane</h1>
            <p class="lead mx-auto text-white-50" style="max-width: 700px;">
                Pemberdayaan Kesejahteraan Keluarga — kelola konten, kepengurusan, unit & program kerja.
            </p>
...
<div class="card shadow-sm border-0 mb-5" id="konten">
    <div class="card-header bg-gradient-bkm-primary text-white border-0">
        <div class="d-flex align-items-center">
            ...
            <div>
                <h5 class="mb-0">Konten Halaman PKK</h5>
            ...
            <div class="row g-4">
                <div class="col-lg-8">
                    <div class="mb-4">
                        ...
                        <textarea name="overview" rows="5" class="form-control" placeholder="Deskripsi singkat tentang PKK..."><?= htmlspecialchars($content['overview']) ?></textarea>
                    </div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label fw-semibold"><i class="fa-solid fa-eye text-primary me-1"></i>Visi</label>
                            <textarea name="visi" rows="3" class="form-control" placeholder="Visi PKK..."><?= htmlspecialchars($content['visi']) ?></textarea>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold"><i class="fa-solid fa-bullseye text-primary me-1"></i>Misi</label>
                            <textarea name="misi" rows="3" class="form-control" placeholder="Misi PKK..."><?= htmlspecialchars($content['misi']) ?></textarea>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="mb-4">
                        ...
                        <textarea name="legal_basis" rows="3" class="form-control" placeholder="Dasar hukum pembentukan..."><?= htmlspecialchars($content['legal_basis']) ?></textarea>
                    </div>
                    <div class="mb-4">
                        <label class="form-label fw-semibold"><i class="fa-solid fa-map-location-dot text-primary me-1"></i>Wilayah Kerja</label>
                        <textarea name="work_area" rows="3" class="form-control" placeholder="Wilayah kerja PKK..."><?= htmlspecialchars($content['work_area']) ?></textarea>
                    </div>
                    <div class="mb-4">
                        <label class="form-label fw-semibold"><i class="fa-solid fa-note-sticky text-primary me-1"></i>Catatan Tambahan</label>
                        <textarea name="notes" rows="2" class="form-control" placeholder="Catatan opsional..."><?= htmlspecialchars($content['notes']) ?></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="card-header bg-gradient-bkm-primary text-white border-0">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <small class="opacity-75">Kepengurusan PKK</small>
                    </div>
                </div>
            </div>
            <div class="card-body">
            ...
            <div class="card-header bg-gradient-bkm-info text-white border-0">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <small class="opacity-75">Bidang-bidang di bawah PKK</small>
                    </div>
                </div>
            </div>
            <div class="card-body">
            ...
            <div class="card-header bg-gradient-bkm-success text-white border-0">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <small class="opacity-75">Program kegiatan PKK</small>
                    </div>
                </div>
            </div>
            <div class="card-body">
...
document.querySelectorAll('.sortable-list').forEach(list => {
    new Sortable(list, {
        ...
        onEnd: function(evt) {
            ...
            fetch('admin_kelembagaan_pkk.php', { method: 'POST', body: formData })
                .then(r => r.json())
                .then(data => {
                    if (data.success) {
