...
$page_key = 'lpmk';
$page_title = 'Kelola Kelembagaan LPMK';
$current_page = 'admin_kelembagaan_lpmk';
...
if (isset($_POST['update_content'])) {
    ...
    $_SESSION['flash_msg'] = 'Konten LPMK berhasil disimpan.';
    header("Location: admin_kelembagaan_lpmk.php?status=terhapus");
    exit;
}
...
if (isset($_POST['save_staff'])) {
    ...
    if ($staff_id) {
        ...
        $_SESSION['flash_msg'] = 'Anggota kepengurusan LPMK berhasil ditambahkan.';
    }
    header("Location: admin_kelembagaan_lpmk.php?status=terhapus");
    exit;
}
...
if (isset($_POST['save_unit'])) {
    ...
    if ($unit_id) {
        ...
        $_SESSION['flash_msg'] = 'Unit pengelola LPMK berhasil ditambahkan.';
    }
    header("Location: admin_kelembagaan_lpmk.php?status=terhapus");
    exit;
}
...
if (isset($_POST['save_program'])) {
    ...
    if ($program_id) {
        ...
        $_SESSION['flash_msg'] = 'Program kerja LPMK berhasil ditambahkan.';
    }
    header("Location: admin_kelembagaan_lpmk.php?status=terhapus");
    exit;
}
...
<div class="container-fluid mt-4">
    <div class="hero-lpmk-admin text-center mb-5">
        <div style="position: relative; z-index: 2;">
            <span class="badge bg-light text-dark mb-3 px-3 py-2 rounded-pill shadow-sm" style="font-size: 0.9rem;">
                <i class="fa-solid fa-building text-danger me-1"></i> Admin Panel
            </span>
            <h1 class="display-5 fw-bold mb-3 text-white">Kelola LPMK Kedungpane</h1>
            <p class="lead mx-auto text-white-50" style="max-width: 700px;">
                Lembaga Pemberdayaan Masyarakat Kelurahan — kelola konten, kepengurusan, unit & program kerja.
            </p>
            <div class="row g-3 mt-4 justify-content-center">
                <div class="col-auto">
                    <div class="status-box-lpmk">
                        <i class="fa-solid fa-users fs-5 text-danger me-2"></i>
                        <span class="fw-bold text-danger">Pengurus</span>
                        <div class="fs-4 fw-bold text-dark mt-2"><?= count($staff) ?></div>
                    </div>
                </div>
                <div class="col-auto">
                    <div class="status-box-lpmk">
                        <i class="fa-solid fa-sitemap fs-5 text-primary me-2"></i>
                        <span class="fw-bold text-primary">Unit</span>
...
<div class="card shadow-sm border-0 mb-5">
    <div class="card-header bg-gradient-maroon text-white border-0">
        <div class="d-flex align-items-center">
            <i class="fa-solid fa-file-lines fs-5 me-3"></i>
            <div>
                <h5 class="mb-0">Konten Halaman LPMK</h5>
    ...
    <div class="card-body">
        <form method="POST">
            <div class="row g-4">
                <div class="col-lg-8">
                    <div class="mb-4">
                        <label class="form-label fw-semibold"><i class="fa-solid fa-align-left text-danger me-1"></i>Overview Singkat</label>
                        <textarea name="overview" rows="5" class="form-control" placeholder="Deskripsi singkat tentang LPMK..."><?= htmlspecialchars($content['overview']) ?></textarea>
                    </div>
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            <label class="form-label fw-semibold"><i class="fa-solid fa-eye text-danger me-1"></i>Visi</label>
                            <textarea name="visi" rows="3" class="form-control" placeholder="Visi LPMK..."><?= htmlspecialchars($content['visi']) ?></textarea>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold"><i class="fa-solid fa-bullseye text-danger me-1"></i>Misi</label>
                            <textarea name="misi" rows="3" class="form-control" placeholder="Misi LPMK..."><?= htmlspecialchars($content['misi']) ?></textarea>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="mb-4">
                        <label class="form-label fw-semibold"><i class="fa-solid fa-scale-balanced text-danger me-1"></i>Dasar Hukum</label>
                    ...
                    <div class="mb-4">
                        <label class="form-label fw-semibold"><i class="fa-solid fa-map-location-dot text-danger me-1"></i>Wilayah Kerja</label>
                        <textarea name="work_area" rows="4" class="form-control" placeholder="Wilayah kerja LPMK..."><?= htmlspecialchars($content['work_area']) ?></textarea>
                    </div>
                </div>
            </div>
            <button type="submit" name="update_content" class="btn btn-danger btn-lg px-5 fw-bold">
            ...
            <div class="card-header bg-gradient-maroon text-white border-0">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <small class="opacity-75">Kepengurusan LPMK</small>
                    </div>
                </div>
            </div>
            <div class="card-body">
                <form method="POST" id="formStaff">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold"><i class="fa-solid fa-user text-danger me-1"></i>Nama</label>
                        <input type="text" name="staff_name" id="staff_name" class="form-control" placeholder="Nama lengkap" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold"><i class="fa-solid fa-id-badge text-danger me-1"></i>Jabatan</label>
                        <input type="text" name="staff_role" id="staff_role" class="form-control" placeholder="Ketua, Sekretaris, dll.">
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold"><i class="fa-solid fa-phone text-danger me-1"></i>Kontak</label>
                        <input type="text" name="staff_contact" id="staff_contact" class="form-control" placeholder="No. HP / email">
                    </div>
                    <button type="submit" name="save_staff" class="btn btn-danger w-100 fw-bold" id="btnSaveStaff">
                        <i class="fa-solid fa-plus me-2"></i>Tambah Anggota
                    </button>
                    <button type="button" id="btnResetStaff" class="btn btn-outline-secondary w-100 mt-2 fw-bold" style="display:none" onclick="resetStaffForm()">
                ...
                <div class="d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <small class="opacity-75">Daftar pengurus LPMK Kedungpane</small>
                        </div>
                    </div>
                    <span class="badge bg-white text-danger fs-6 px-3 py-2">
                        <i class="fa-solid fa-user me-1"></i><?= count($staff) ?>
                    </span>
                </div>
            </div>
            <div class="card-body">
               <?php if(count($staff)): ?>
                <div class="sortable-list" id="sortable-staff" data-table="kelembagaan_staff">
                    <?php foreach($staff as $index => $row): ?>
                    <div class="sortable-item staff-sortable-item" data-id="<?= $row['id'] ?>">
                        <div class="drag-handle" title="Seret untuk mengubah urutan"><i class="fa-solid fa-grip-vertical fa-lg"></i></div>
                        <img src="https://ui-avatars.com/api/?name=<?= urlencode($row['name']) ?>&background=dc3545&color=fff&size=44" class="rounded-circle flex-shrink-0" alt="<?= htmlspecialchars($row['name']) ?>" style="width:44px;height:44px;">
                        <div class="item-info">
                            <div class="item-name"><?= htmlspecialchars($row['name']) ?></div>
                            <div class="item-addr">
                                <span class="badge bg-danger-subtle text-danger border border-danger-subtle me-1" style="font-size:.7rem"><?= htmlspecialchars($row['role'] ?: 'Anggota') ?></span>
                                <?php if(!empty($row['contact'])): ?><i class="fa-solid fa-phone me-1 text-muted"></i><?= htmlspecialchars($row['contact']) ?><?php endif; ?>
                            </div>
                        </div>
                        <div class="item-actions d-flex gap-1">
                            <button type="button" class="btn btn-sm btn-light text-primary rounded-circle shadow-sm edit-staff" data-id="<?= $row['id'] ?>" data-name="<?= htmlspecialchars($row['name'], ENT_QUOTES) ?>" data-role="<?= htmlspecialchars($row['role'], ENT_QUOTES) ?>" data-contact="<?= htmlspecialchars($row['contact'], ENT_QUOTES) ?>" title="Edit"><i class="fa-solid fa-pencil"></i></button>
                            <a href="?delete_staff=<?= $row['id'] ?>" class="btn btn-sm btn-light text-danger rounded-circle shadow-sm btn-hapus" title="Hapus"><i class="fa-solid fa-trash-can"></i></a>
                        </div>
                    </div>
                    <?php endforeach; ?>
                </div>
                ...
                <div class="text-center py-5">
                    ...
                    <h5 class="text-muted">Belum ada pengurus</h5>
                    <p class="text-muted">Tambahkan pengurus menggunakan form di sebelah kiri</p>
                </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <div class="col-lg-6">
            ...
            <div class="card-header bg-gradient-primary text-white border-0">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <small class="opacity-75">Bidang-bidang di bawah LPMK</small>
                    </div>
                </div>
            </div>
            <div class="card-body">
                <form method="POST" id="formUnit">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold"><i class="fa-solid fa-tag text-primary me-1"></i>Judul Unit</label>
                        <input type="text" name="unit_title" id="unit_title" class="form-control" placeholder="Nama unit" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold"><i class="fa-solid fa-circle-info text-primary me-1"></i>Deskripsi</label>
                        <textarea name="unit_description" id="unit_description" rows="3" class="form-control" placeholder="Deskripsi tugas unit..."></textarea>
                    </div>
                    <button type="submit" name="save_unit" class="btn btn-primary w-100 fw-bold" id="btnSaveUnit">
                        <i class="fa-solid fa-plus me-2"></i>Tambah Unit
                    </button>
                    <button type="button" id="btnResetUnit" class="btn btn-outline-secondary w-100 mt-2 fw-bold" style="display:none" onclick="resetUnitForm()">
                        <i class="fa-solid fa-xmark me-1"></i>Batal Edit
                    </button>
                </form>
            </div>
        ...
        <div class="card shadow-sm border-0">
            <div class="card-header bg-gradient-primary text-white border-0">
                <div class="d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center">
                        <i class="fa-solid fa-list fs-5 me-3"></i>
                        <h5 class="mb-0">Daftar Unit Pengelola</h5>
                    </div>
                    <span class="badge bg-white text-primary fs-6 px-3 py-2"><?= count($units) ?></span>
                </div>
            </div>
            <div class="card-body">
                <p class="text-muted small mb-3"><i class="fa-solid fa-grip-vertical me-1"></i><strong>Seret</strong> untuk mengubah urutan.</p>
                <?php if(count($units)): ?>
                <div class="sortable-list" id="sortable-units" data-table="kelembagaan_units">
                    <?php foreach($units as $i => $unit): ?>
                    <div class="sortable-item unit-card" data-id="<?= $unit['id'] ?>">
                        <div class="drag-handle" title="Seret"><i class="fa-solid fa-grip-vertical fa-lg"></i></div>
                        <div class="item-info">
                            <div class="item-name"><?= htmlspecialchars($unit['title']) ?></div>
                            <div class="item-addr"><?= htmlspecialchars($unit['description']) ?></div>
                        </div>
                        <div class="item-actions d-flex gap-1">
                            <button type="button" class="btn btn-sm btn-light text-primary rounded-circle shadow-sm edit-unit" data-id="<?= $unit['id'] ?>" data-title="<?= htmlspecialchars($unit['title'], ENT_QUOTES) ?>" data-description="<?= htmlspecialchars($unit['description'], ENT_QUOTES) ?>" title="Edit"><i class="fa-solid fa-pencil"></i></button>
                            <a href="?delete_unit=<?= $unit['id'] ?>" class="btn btn-sm btn-light text-danger rounded-circle shadow-sm btn-hapus" title="Hapus"><i class="fa-solid fa-trash-can"></i></a>
                        </div>
                    </div>
                    <?php endforeach; ?>
                </div>
                ...
                <div class="text-center py-4">
                    <i class="fa-solid fa-folder-open fs-1 text-muted mb-2 d-block"></i>
    ...
    <div class="col-lg-6">
            ...
            <div class="card-header bg-gradient-success text-white border-0">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <small class="opacity-75">Program kegiatan LPMK</small>
                    </div>
                </div>
            </div>
            <div class="card-body">
                <form method="POST" id="formProgram">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold"><i class="fa-solid fa-tag text-success me-1"></i>Judul Program</label>
                        <input type="text" name="program_title" id="program_title" class="form-control" placeholder="Nama program" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold"><i class="fa-solid fa-circle-info text-success me-1"></i>Deskripsi</label>
                    ...
                    <button type="submit" name="save_program" class="btn btn-success w-100 fw-bold" id="btnSaveProgram">
                        <i class="fa-solid fa-plus me-2"></i>Tambah Program
                    </button>
                    <button type="button" id="btnResetProgram" class="btn btn-outline-secondary w-100 mt-2 fw-bold" style="display:none" onclick="resetProgramForm()">
                        <i class="fa-solid fa-xmark me-1"></i>Batal Edit
                    </button>
                </form>
            </div>
        ...
        <div class="card shadow-sm border-0">
            <div class="card-header bg-gradient-success text-white border-0">
                <div class="d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center">
                        <i class="fa-solid fa-list-check fs-5 me-3"></i>
                        <h5 class="mb-0">Daftar Program Kerja</h5>
                    </div>
                    <span class="badge bg-white text-success fs-6 px-3 py-2"><?= count($programs) ?></span>
                </div>
            </div>
            <div class="card-body">
                <p class="text-muted small mb-3"><i class="fa-solid fa-grip-vertical me-1"></i><strong>Seret</strong> untuk mengubah urutan.</p>
                <?php if(count($programs)): ?>
                <div class="sortable-list" id="sortable-programs" data-table="kelembagaan_programs">
                    ...
                    <div class="sortable-item program-card" data-id="<?= $prog['id'] ?>">
                        ...
                        <div class="item-info">
                            ...
                            <div class="item-addr"><?= htmlspecialchars($prog['description']) ?></div>
                        </div>
                        <div class="item-actions d-flex gap-1">
                            <button type="button" class="btn btn-sm btn-light text-primary rounded-circle shadow-sm edit-program" data-id="<?= $prog['id'] ?>" data-title="<?= htmlspecialchars($prog['title'], ENT_QUOTES) ?>" data-description="<?= htmlspecialchars($prog['description'], ENT_QUOTES) ?>" title="Edit"><i class="fa-solid fa-pencil"></i></button>
                            <a href="?delete_program=<?= $prog['id'] ?>" class="btn btn-sm btn-light text-danger rounded-circle shadow-sm btn-hapus" title="Hapus"><i class="fa-solid fa-trash-can"></i></a>
                        </div>
                    </div>
                    <?php endforeach; ?>
                </div>
...
<style>
.hero-lpmk-admin {
    background: linear-gradient(135deg, rgba(74,0,0,0.92), rgba(128,0,0,0.88)), url('https://images.unsplash.com/photo-1577495508048-b635879837f1?auto=format&fit=crop&w=1200&q=80') center/cover no-repeat;
...
.status-box-lpmk {
    ...
    border-top: 4px solid var(--maroon-bright, #800000); transition: transform 0.3s ease; min-width: 120px;
}
.status-box-lpmk:hover { transform: translateY(-5px); }
.bg-gradient-maroon { background: linear-gradient(135deg, #800000 0%, #4a0000 100%); }
...
.unit-card { border-left: 4px solid #667eea !important; }
.unit-card:hover { border-left-color: #764ba2 !important; box-shadow: 0 8px 25px rgba(102,126,234,0.15) !important; }
.program-card { border-left: 4px solid #11998e !important; }
.program-card:hover { border-left-color: #38ef7d !important; box-shadow: 0 8px 25px rgba(17,153,142,0.15) !important; }
.staff-sortable-item { border-left: 4px solid #dc3545 !important; }
.staff-sortable-item:hover { border-left-color: #b02a37 !important; box-shadow: 0 8px 25px rgba(220,53,69,0.12) !important; }
...
document.querySelectorAll('.sortable-list').forEach(list => {
    new Sortable(list, {
        ...
        onEnd: function(evt) {
            ...
            fetch('admin_kelembagaan_lpmk.php', { method: 'POST', body: formData })
                .then(r => r.json())
                .then(data => {
                    if (data.success) {
